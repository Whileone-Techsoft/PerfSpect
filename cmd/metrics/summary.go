...
func (m *metricsFromCSV) loadHTMLTemplateValues(metadata Metadata) (templateVals map[string]string, err error) {
	...
	archIndex := 0
	//slog.Debug("Before", slog.Int("archIndex",archIndex))
	fmt.Println("Value of archIndex frst is:", archIndex)
	...
	//		archIndex = 2
	//}
	//slog.Debug("After" ,slog.Int("archIndex",archIndex))	
	fmt.Println("Value archIndex after is:", archIndex)
...
g}
	...
	for _, tmpl := range templateReplace {
		...
		metricMean := 0.0
		fmt.Println("Len is :", len(tmpl.metricNames))
		fmt.Println("stat is: ",stats)
		if len(tmpl.metricNames) > archIndex {
